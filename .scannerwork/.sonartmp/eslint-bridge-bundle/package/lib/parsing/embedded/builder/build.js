 ...
 */
function buildSourceCodes(input, language) {
    let embeddedJSs;
    if (language === 'html') {
        embeddedJSs = (0, embedded_1.parseHTML)(input.fileContent);
    }
    else {
        embeddedJSs = (0, embedded_1.parseAwsFromYaml)(input.fileContent);
    }
    ...
    for (const embeddedJS of embeddedJSs) {
        ...
        try {
            const sourceCode = (0, jsts_1.buildSourceCode)(jsTsAnalysisInput);
