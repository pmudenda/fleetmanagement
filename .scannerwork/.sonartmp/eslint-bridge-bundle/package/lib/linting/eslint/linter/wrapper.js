...
class LinterWrapper {
    ...
    lint(sourceCode, filePath, fileType = 'MAIN', language = 'js') {
        ...
        (0, helpers_1.debug)(`Using linter configuration for ${JSON.stringify(key)}`);
    ...
    createConfig() {
        var _a;
        (0, helpers_1.debug)('Creating linter config');
        const rulesByKey = new Map();
        (_a = this.options.inputRules) === null || _a === void 0 ? void 0 : _a.forEach(r => {
            const target = Array.isArray(r.fileTypeTarget) ? r.fileTypeTarget : [r.fileTypeTarget];
            target.forEach(fileType => {
                var _a, _b;
                const key = this.linterConfigurationKey({ language: (_a = r.language) !== null && _a !== void 0 ? _a : 'js', fileType });
                const rules = (_b = rulesByKey.get(key)) !== null && _b !== void 0 ? _b : [];
                rules.push(r);
                rulesByKey.set(key, rules);
            });
        });
        rulesByKey.forEach((rules, key) => {
            (0, helpers_1.debug)(`Linter config: ${JSON.stringify(key)} with ${rules
                .map(r => r.key)
