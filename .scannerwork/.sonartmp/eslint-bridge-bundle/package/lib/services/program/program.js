...
exports.isRoot = exports.isRootNodeModules = exports.createProgram = exports.createProgramOptions = exports.getProgramForFile = exports.getDefaultTSConfigs = exports.setDefaultTSConfigs = exports.programCache = void 0;
const path_1 = __importDefault(require("path"));
const typescript_1 = __importDefault(require("typescript"));
const helpers_1 = require("helpers");
const cache_1 = require("helpers/cache");
exports.programCache = new cache_1.ProgramCache();
const projectTSConfigsByBaseDir = new Map();
function setDefaultTSConfigs(baseDir, tsConfigs) {
    // used only in tests
    projectTSConfigsByBaseDir.set(baseDir, tsConfigs);
}
exports.setDefaultTSConfigs = setDefaultTSConfigs;
function getDefaultTSConfigs(baseDir, inputTSConfigs) {
    let tsConfigs = projectTSConfigsByBaseDir.get(baseDir);
    if (!tsConfigs) {
        tsConfigs = new helpers_1.ProjectTSConfigs(baseDir, inputTSConfigs);
        projectTSConfigsByBaseDir.set(baseDir, tsConfigs);
    }
    return tsConfigs;
}
exports.getDefaultTSConfigs = getDefaultTSConfigs;
/**
...
function getProgramForFile(input, cache = exports.programCache, tsconfigs) {
     ...
     */
    const topDir = process.env['SONARJS_LIMIT_DEPS_RESOLUTION'] === '1' ? (0, helpers_1.toUnixPath)(input.baseDir) : undefined;
    if (!tsconfigs) {
        ...
        try {
            if (!tsconfig.isFallbackTSConfig) {
                ...
                if (programResult.files.includes(normalizedPath)) {
                    ...
                    cache.mark(program);
                    (0, helpers_1.debug)(`Analyzing ${input.filePath} using tsconfig ${tsconfig.filename}`);
                    return program;
                }
            }
            else {
                ...
                for (const [tsConfigPath, programResult] of cache.getPrograms()) {
                    if (programResult.files.includes(normalizedPath) && programResult.isFallbackProgram) {
                        ...
                        if (program) {
                            cache.mark(program);
                            (0, helpers_1.debug)(`Analyzing file ${input.filePath} using tsconfig ${tsConfigPath}`);
                            return program;
                        }
                        else {
                ...
                if (programResult.files.includes(normalizedPath)) {
                    const program = programResult.program.deref();
                    cache.mark(program);
                    (0, helpers_1.debug)(`Analyzing file ${input.filePath} using fallback tsconfig ${tsconfig.filename}`);
                    return program;
                }
            }
        }
        catch (e) {
            console.log(`ERROR: Failed create program with tsconfig ${tsconfig.filename}}. Error: ${e.message}`);
        }
    }
 ...
 */
function createProgramOptions(tsConfig, tsconfigContents, topDir) {
    ...
    const parseConfigHost = {
        ...
        fileExists: file => {
            ...
            if (topDir && !file.startsWith((0, helpers_1.toUnixPath)(topDir))) {
                return false;
            }
 ...
 */
function createProgram(tsConfig, tsconfigContents, topDir) {
    if (!tsconfigContents) {
    ...
    if (topDir) {
        programOptions.host = typescript_1.default.createCompilerHost(programOptions.options);
        ...
        programOptions.host.fileExists = fileName => {
            if (path_1.default.isAbsolute(fileName) && !fileName.startsWith(topDir)) {
                return false;
            }
    ...
    (0, helpers_1.debug)(`Created program for ${tsConfig} with ${files.length} files`);
    return {
        files,
        projectReferences,
 ...
 */
function isLastTsConfigCheck(file, topDir) {
    return path_1.default.basename(file) === 'tsconfig.json' && isRootNodeModules(file, topDir);
}
function isRootNodeModules(file, topDir) {
    if (!topDir) {
        topDir = path_1.default.parse(file).root;
    }
    const normalizedFile = (0, helpers_1.toUnixPath)(file);
    const topNodeModules = (0, helpers_1.toUnixPath)(path_1.default.resolve(path_1.default.join(topDir, 'node_modules')));
