...
class FuelStatusByAmountChart extends Component {
...
"))->groupBy('name');
        ...
        $data = $data->map(function ($app) {
            return (double)$app->sum('amt');
        })->values()->toArray();
        return view('livewire.reports.fuel.status.fuel-status-by-amount-chart', compact('data', 'categories'));
    }
}
