...
//        dd(compact('maximumDistance','minDistance','quantityLastIssued','odometerOnLastIssue','newEstimatedOdometer'));
        ...
        Log::debug("Last Issue + Min Odometer On Last Issue " . $newEstimatedOdometer);
        Log::debug("Last Issue + Max Odometer On Last Issue " . $maximumDistance + $odometerOnLastIssue);
    ...
    public function getMyRequisitions($staff_no, $search = false): Collection
    {
        if ($staff_no) {
            return DB::table("GEN_MATERIAL_HEADERS")
                ...
                ->whereDate('mat_head.CREATED_AT', '>=', now()->subQuarters(1)->toDateString())
                ->select(
