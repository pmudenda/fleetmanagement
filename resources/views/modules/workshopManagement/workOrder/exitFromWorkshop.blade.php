@php
    use App\Enums\RequisitionItemTypes;use App\Helpers\StatusHelper;use App\Helpers\VehicleStatus;use Carbon\Carbon;
@endphp
@extends('layouts.app')
@push('styles')
                                                        ...
                                                        <div class="col-xs-12 col-sm-12 col-md-7 col-lg-7">
                                                            <input type="text"
                                                                   ...
                                                                   date('Y-m-d', strtotime(
                                                                       Carbon::parse($details->date_in
                                                                       )->format('Y-m-d')))}}
                                                                   @else{{date('Y-m-d', strtotime(
                                                                   Carbon::now()))}}@endif"
                                                                   name="date_of_req"
                                                            >
                                                        </div>
                                                                    ...
                                                                    <tr class="increment">
                                                                        ...
                                                                        <td>
                                                                            <select name="workshopSection"
                                                                                    ...
                                                                                    workshopSection">
                                                                                <option></option>
                                                                                @foreach($workshopSections
                                                                                    as $workshopSection)
                                                                                    @if($defect->section_code
                                                                                        == $workshopSection->code)
                                                                                        <option selected
                                                                                                value="{{
                                                                                                $workshopSection->code
                                                                                            }}">
                                                                ...
                                                                <tr class="increment"
                                                                    ...
                                                                    <td>
                                                                        <input type="text"
                                                                               ...
                                                                               class="form-control
                                                                               form-control-sm technical_specification"
                                                                        />
                                                                    </td>
                                                                    ...
                                                                    <td>
                                                                        <input
                                                                                ...
                                                                                readonly
                                                                                class="form-control
                                                                                form-control-sm unit_of_measure"/>
                                                                    </td>
                                                                    ...
                                                                    <td>
                                                                        <span
                                                                                id="total_price">{{$material->amount ?? ''}}
                                                                        </span>
...
->format('d/m/Y')}}"
                                        ...
                                        <div class="col-xs-12 col-sm-12 col-md-12 px-0">
                                            ...
                                            <div class="row">
                                                @if(!empty($comments)
                                                    && !empty($comments->where('type','=','SREQ')->first())
                                                    )
                                                    <div class="form-group">
                                                        <label
                                                                class="col-xs-12 col-sm-6
...
@push('scripts')
    ...
    <script>
        ...
        (function (tmsApp, $) {
            ...
            function loadData(key, url, defectSelectionElem) {
                fetch(url)
                    .then(response => response.json())
                    .then(response => {
                        ...
                        let location = defectSelectionElem.attr('data-value');

                        if (location) {
                            defectSelectionElem.val(location);
                            defectSelectionElem.trigger('change');
                        }
                    })
                    .catch(function (error) {
                        toastr.error(
                            'Could not retrieve fuel levels data, some feature might not work.',
                            'Defects Data'
                        );
                    });
            }
            ...
            function displayVehicleDetails(payload, state) {
                ...
                tmsApp.asyncGetFormData(
                    ...
                    function (response_data) {
                        if (response_data.success === 'true' || response_data.success === true) {
                            displayVehicleDetails(response_data.payload, stage);
                        } else {
                            removeSubmissionAndDetailsOptions();
                            tmsApp.systemError(
                                'Vehicle',
                ...
                switch (element.name) {
                    ...
                    case 'ratePerHour':
                        let lineTotal = tmsApp.getFloat($(element).closest("tr").find("input[name=hoursWorked]").val())
                            * tmsApp.getFloat($(element).closest("tr")
                                .find("input[name=ratePerHour]").val());

                        $(element).closest("tr")
                            .find("input[name=totalAmount]")
                            .val(tmsApp.numberFormat(lineTotal))
                            .change();
                        break;
                    ...
                    case 'service_unit_price':
                        let serviceTotalAmount = tmsApp.getFloat(element.value)
                            * tmsApp.getFloat($(element).closest("tr")
                                .find("input[name=service_quantity]").val());
            ...
            function setSelectedAccessories() {
                ...
                $.each(selectedAccessories, function (index, element) {
                    ...
                    $("input[name=field_" + element?.code + "][value=" + otherResponse + "]").attr('disabled', true);
                        ...
                        $(row).find('input[name], select[name]').each(function (i, item) {
                            ...
                            if (item.name === 'endDate' || item.name === 'startDate' || item.name === 'invoiceDate') {
                                ...
                                dateField = DateFormatter.format(
                                    new Date(moment(val, 'DD/MM/yyyy')), DateFormatter.ISO
                                );
            ...
            loadData('VEH_SYS',
                document.querySelector('#systemsUrl').value + '?key=VEH_SYS', $('select[name="vehicleSystem"]')
            );
