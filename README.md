...
## System Scripts
```oracle
create FUNCTION fn_create_stores_req (
...
---------------------------------------------------------------------
    ...
    SELECT
        ...
        '0'           AS ind_project, -- ind_project
        d.project_code --d.project_no
    FROM
...
--------------------------------------------------------------------------------
    ...
    LOOP
        ...
        IF v_req_article_group = '300101' THEN
            IF v_req_project_ind = '01' THEN  -- 01 means cost center
                SELECT
                    bu_code,
                    cc_code
                INTO
...
--------------------------------------------------------------------------------
        ...
        /* FUEL REQUISTION ROK ZMES-892 nvl ()replace null with 01 */
        IF v_req_article_group = '300101' THEN
            v_fuel_req_ind := 1;
            SELECT
                COUNT(*),
...
--
        ls_point := '1.9';
        IF rec_spare_trans.system_origin = '04' THEN --dcs
            ls_function := '';
        ...
        IF ll_count < 1 THEN
            ...
            ls_return := storesdocumentnumbergenerator('SEQ_STORE_REQ', ls_area);
